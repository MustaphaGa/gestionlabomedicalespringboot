...
public class PreleveurValidator {
	...
	public static List<String> validate(PreleveurDTO preleveurDTO){
		...
		if(preleveurDTO == null) {
			...
			errors.add("veuillez renseigner login de preleveur!!");
			errors.add("veuillez renseigner le password de preleveur!!");
        ...
        if(!StringUtils.hasLength(preleveurDTO.getLogin())) {
        	errors.add("veuillez renseigner login de preleveur!!");
		}
        
        if(!StringUtils.hasLength(preleveurDTO.getPassword())) {
        	errors.add("veuillez renseigner le password de preleveur!!");
		}
