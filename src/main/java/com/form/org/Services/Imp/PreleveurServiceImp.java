...
public class PreleveurServiceImp implements PreleveurService{
	...
	@Override
	public PreleveurDTO findById(Integer id) {
		if(id == null) {	
			log.error("preleveur id is null");
			return null;
		}
		Optional<Preleveur> preleveur=preleveurRepositoy.findById(id);
		
		return Optional.of(PreleveurDTO.fromEntity(preleveur.get())).orElseThrow(() ->
		       new EntityNotFoundException(
				"Aucun patient avec l'ID ="+ id +"n'été trouve dans la BDD",
	...
	public void delete(Integer id) {
		...
		preleveurRepositoy.deleteById(id);
