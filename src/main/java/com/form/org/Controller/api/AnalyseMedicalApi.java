...
@Api(APP_ROOT +"/analyseMedicals")
public interface AnalyseMedicalApi {
	
	@PostMapping(value= APP_ROOT + "/analyseMedical/create", consumes = MediaType.APPLICATION_JSON_VALUE , produces = MediaType.APPLICATION_JSON_VALUE)
	    ...
	    @ApiResponses(value = {
		@ApiResponse(code = 2000, message = "l'objet analyse medical cree / modifie")	
	})
	AnalyseMedicalDTO save(AnalyseMedicalDTO dto);
	
	@GetMapping(value= APP_ROOT + "/analyseMedical/{idAnalyseMedical}", produces = MediaType.APPLICATION_JSON_VALUE )
	...
	@ApiResponses(value = {
		@ApiResponse(code = 2000, message = "l'analyse medical a ete trouver dans la BDD"),	
	...
	@GetMapping(value= APP_ROOT + "/analyseMedical/all", produces = MediaType.APPLICATION_JSON_VALUE )
	 ...
	 responseContainer = "List<AnalyseMedicalDTO>")
		@ApiResponses(value = {
			@ApiResponse(code = 2000, message = "la liste d'analyse medical / une liste vide"),		
			})
	...
	@ApiOperation(value = "supprimer  l'analyse Medical ",notes = "Cette methode permet de supprimer l'analyse Medical par ID ",
	 response = AnalyseMedicalDTO.class)
	    @ApiResponses(value = {
		@ApiResponse(code = 2000, message = "analyse Medical a ete supprimer"),	
	    })
	   void delecte(Integer idAnalyseMedical);
