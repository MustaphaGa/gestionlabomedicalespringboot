...
public interface PatientApi{
	...
	@PostMapping(value= APP_ROOT + "/patients/create", consumes = MediaType.APPLICATION_JSON_VALUE , produces = MediaType.APPLICATION_JSON_VALUE )
	 ...
	 + "d'enregistrer ou modifier un patient ", response = PatientDTO.class)
	  @ApiResponses(value = {
	  @ApiResponse(code = 2000, message = "l'objet patient cree / modifie")	
				})
	...
	@ApiResponses(value = {
	 @ApiResponse(code = 2000, message = "la patient a ete trouver dans la BDD"),	
	...
	@GetMapping(value= APP_ROOT + "/patients/{codePatient}", produces = MediaType.APPLICATION_JSON_VALUE )
	...
	@DeleteMapping(value =APP_ROOT + "patients/delete/{idPatient}" )
	 @ApiOperation(value = "supprimer  un patient ",notes = "Cette methode permet de supprimer"
		     + " un patient par ID ",
		 	 response = PatientDTO.class)
		 	  @ApiResponses(value = {
		 	  @ApiResponse(code = 2000, message = "patient a ete supprimer"),	
		 		})
	void delecte(@PathVariable("idPatient") Integer id);
